import Box from "@mui/material/Box";
import theme from "../../theme";
import Typography from "@mui/material/Typography";
import { LineChart } from "@mui/x-charts/LineChart";
import { Badge } from "react-bootstrap";

import FilterAltIcon from "@mui/icons-material/FilterAlt";
import ButtonMUI from "@mui/material/Button";
import Menu from "@mui/material/Menu";
import MenuItem from "@mui/material/MenuItem";

import { useState } from "react";
import { useContext, useEffect } from "react";
import PlantContext from "../../context/PlantContext";
import {
  doc,
  query,
  collection,
  orderBy,
  limit,
  onSnapshot,
  getDoc,
} from "firebase/firestore";

function Dashboard() {
  const [anchorEl, setAnchorEl] = useState(null);
  const open = Boolean(anchorEl);
  const handleClick = (event) => {
    setAnchorEl(event.currentTarget);
  };
  const handleClose = () => {
    setAnchorEl(null);
  };
  const { plantList, sendRequest } = useContext(PlantContext);

  useEffect(() => {
    sendRequest(null, "get_all_plant");
  }, []);
  const dataset = [
    { date: new Date(1990, 0, 1), fr: 28129, gb: 26189, dl: 25391 },
    { date: new Date(1991, 0, 1), fr: 28294.264, gb: 25792.014, dl: 26769.96 },
    { date: new Date(1992, 0, 1), fr: 28619.805, gb: 25790.186, dl: 27385.055 },
    { date: new Date(1993, 0, 1), fr: 28336.16, gb: 26349.342, dl: 27250.701 },
    { date: new Date(1994, 0, 1), fr: 28907.977, gb: 27277.543, dl: 28140.057 },
    { date: new Date(1995, 0, 1), fr: 29418.863, gb: 27861.215, dl: 28868.945 },
    { date: new Date(1996, 0, 1), fr: 29736.645, gb: 28472.248, dl: 29349.982 },
    { date: new Date(1997, 0, 1), fr: 30341.807, gb: 29259.764, dl: 30186.945 },
    { date: new Date(1998, 0, 1), fr: 31323.078, gb: 30077.385, dl: 31129.584 },
    { date: new Date(1999, 0, 1), fr: 32284.611, gb: 30932.537, dl: 32087.604 },
    { date: new Date(2000, 0, 1), fr: 33409.68, gb: 31946.037, dl: 33367.285 },
    { date: new Date(2001, 0, 1), fr: 33920.098, gb: 32660.441, dl: 34260.29 },
    { date: new Date(2002, 0, 1), fr: 34152.773, gb: 33271.3, dl: 34590.93 },
    { date: new Date(2003, 0, 1), fr: 34292.03, gb: 34232.426, dl: 34716.44 },
    { date: new Date(2004, 0, 1), fr: 35093.824, gb: 34865.78, dl: 35528.715 },
    { date: new Date(2005, 0, 1), fr: 35495.465, gb: 35623.625, dl: 36205.574 },
    { date: new Date(2006, 0, 1), fr: 36166.16, gb: 36214.07, dl: 38014.137 },
    { date: new Date(2007, 0, 1), fr: 36845.684, gb: 36816.676, dl: 39752.207 },
    { date: new Date(2008, 0, 1), fr: 36761.793, gb: 36264.79, dl: 40715.434 },
    { date: new Date(2009, 0, 1), fr: 35534.926, gb: 34402.36, dl: 38962.938 },
    { date: new Date(2010, 0, 1), fr: 36086.727, gb: 34754.473, dl: 41109.582 },
    { date: new Date(2011, 0, 1), fr: 36691, gb: 34971, dl: 43189 },
    { date: new Date(2012, 0, 1), fr: 36571, gb: 35185, dl: 43320 },
    { date: new Date(2013, 0, 1), fr: 36632, gb: 35618, dl: 43413 },
    { date: new Date(2014, 0, 1), fr: 36527, gb: 36436, dl: 43922 },
    { date: new Date(2015, 0, 1), fr: 36827, gb: 36941, dl: 44293 },
    { date: new Date(2016, 0, 1), fr: 37124, gb: 37334, dl: 44689 },
    { date: new Date(2017, 0, 1), fr: 37895, gb: 37782.83, dl: 45619.785 },
    { date: new Date(2018, 0, 1), fr: 38515.918, gb: 38058.086, dl: 46177.617 },
  ];

  const { initializeFirestore } = useContext(PlantContext);
  const [selectedPlant, setSelectedPlant] = useState(null);
  const [temperature, setTemperature] = useState(null);
  const [desiredRange, setDesiredRange] = useState(null);
  const [light, setLight] = useState(null);
  const [lightRange, setLightRange] = useState(null);
  const [moisture, setMoisture] = useState(null);
  const [moistureRange, setMoistureRange] = useState(null);
  const [soilHumidity, setSoilHumidity] = useState(null);
  const [soilHumidityRange, setSoilHumidityRange] = useState(null);

  useEffect(() => {
    if (selectedPlant) {
      const db = initializeFirestore();

      // Fetch ranges
      const fetchRanges = async () => {
        const planDocRef = doc(db, "Plant_Plan", selectedPlant.SerialID);
        try {
          const docSnap = await getDoc(planDocRef);
          if (docSnap.exists()) {
            const planData = docSnap.data().Plan;
            if (planData?.StatRanges) {
              setDesiredRange(planData.StatRanges.Temperature);
              setLightRange(planData.StatRanges.Light);
              setMoistureRange(planData.StatRanges.Moisture);
              setSoilHumidityRange(planData.StatRanges.SoilHumidity);
            }
          }
        } catch (error) {
          console.error("Error fetching plant plan:", error);
        }
      };
      fetchRanges();

      // Subscribe to realtime updates
      const unsubTemp = onSnapshot(
        query(
          collection(db, "Plant_Temperature", selectedPlant.SerialID, "Logs"),
          orderBy("Time", "desc"),
          limit(1)
        ),
        (snapshot) => {
          if (!snapshot.empty) {
            setTemperature(snapshot.docs[0].data().Value);
          }
        }
      );

      const unsubLight = onSnapshot(
        query(
          collection(db, "Plant_Light", selectedPlant.SerialID, "Logs"),
          orderBy("Time", "desc"),
          limit(1)
        ),
        (snapshot) => {
          if (!snapshot.empty) {
            setLight(snapshot.docs[0].data().Value);
          }
        }
      );

      const unsubMoisture = onSnapshot(
        query(
          collection(db, "Plant_Moisture", selectedPlant.SerialID, "Logs"),
          orderBy("Time", "desc"),
          limit(1)
        ),
        (snapshot) => {
          if (!snapshot.empty) {
            setMoisture(snapshot.docs[0].data().Value);
          }
        }
      );

      const unsubSoil = onSnapshot(
        query(
          collection(db, "Plant_SoilHumidity", selectedPlant.SerialID, "Logs"),
          orderBy("Time", "desc"),
          limit(1)
        ),
        (snapshot) => {
          if (!snapshot.empty) {
            setSoilHumidity(snapshot.docs[0].data().Value);
          }
        }
      );

      return () => {
        unsubTemp();
        unsubLight();
        unsubMoisture();
        unsubSoil();
      };
    }
  }, [selectedPlant, initializeFirestore]);

  return (
    <Box
      sx={{
        height: theme.trello.homeHeight,
        width: "100%",
        overflowY: "auto",
        backgroundColor: "background.main", // Optional: Set a background color,
        display: "flex",
        flexWrap: "wrap",
        gap: 10,
        p: 4,
        px: 4,
      }}
    >
      {/* Box tren */}
      <Box
        sx={{
          width: "100%",
          display: "flex",
          flexWrap: "wrap",
          justifyContent: "center",
          gap: 5,
          minHeight: "150px",
        }}
      >
        <Box
          sx={{
            minWidth: "250px",
            border: "5px solid",
            borderColor: "primary.main",
            borderRadius: 3,
            display: "flex",
            flexDirection: "column",
            justifyContent: "space-between",
            alignItems: "center",
          }}
        >
          <Typography variant="h5" gutterBottom>
            Healthy Plants
          </Typography>
          <Typography
            variant="h2"
            gutterBottom
            sx={{ color: "primary.main", fontWeight: "bold" }}
          >
            3
          </Typography>
        </Box>
        <Box
          sx={{
            minWidth: "250px",
            border: "5px solid",
            borderColor: "primary.main",
            borderRadius: 3,
            display: "flex",
            flexDirection: "column",
            justifyContent: "space-between",
            alignItems: "center",
          }}
        >
          <Typography variant="h5" gutterBottom>
            Unhealthy Plants
          </Typography>
          <Typography
            variant="h2"
            gutterBottom
            sx={{ color: "#FF0B55", fontWeight: "bold" }}
          >
            3
          </Typography>
        </Box>
        <Box
          sx={{
            borderRadius: 3,
            border: "5px solid",
            borderColor: "primary.main",
            backgroundColor: "#e6e6e6",
            flexGrow: 1,
            minHeight: "150px",
          }}
        >
          <Typography variant="h4" gutterBottom>
            Calendar
          </Typography>
        </Box>
      </Box>

      {/* Box duoi */}
      <Box
        sx={{
          width: "100%",
          display: "flex",
          flexWrap: "wrap",
          justifyContent: "center",
          gap: 10,
        }}
      >
        {/* Unhealthy Plants List */}
        <Box
          sx={{
            width: "500px",
            border: "1px solid #ddd",
            borderRadius: 3,
            overflow: "hidden",
            height: "550px",
            boxShadow: "1px 1px 3px",
          }}
        >
          {/* Title */}
          <Box
            sx={{
              background: "#e6e6e6",
              width: "100%",
              display: "flex",
              alignItems: "center",
              flexWrap: "wrap",
              padding: "8px 16px",
            }}
          >
            <Typography
              sx={{
                display: "flex",
                alignItems: "center",
                color: "#FF0B55",
                fontWeight: "bold",
              }}
              variant="h5"
            >
              Unhealthy Plants List
            </Typography>

            <Box
              sx={{
                display: "flex",
                gap: 2,
                ml: "auto",
                flexWrap: "wrap",
                justifyContent: "flex-end",
                alignItems: "center",
              }}
            >
              <ButtonMUI
                id="basic-button"
                aria-controls={open ? "basic-menu" : undefined}
                aria-haspopup="true"
                aria-expanded={open ? "true" : undefined}
                onClick={handleClick}
              >
                <FilterAltIcon />
              </ButtonMUI>
              <Menu
                id="basic-menu"
                anchorEl={anchorEl}
                open={open}
                onClose={handleClose}
                MenuListProps={{
                  "aria-labelledby": "basic-button",
                }}
              >
                <MenuItem>All</MenuItem>
                <MenuItem>Temperature</MenuItem>
                <MenuItem>Light</MenuItem>
                <MenuItem>Moisture</MenuItem>
                <MenuItem>Soil Humidity</MenuItem>
              </Menu>
            </Box>
          </Box>
          {/* Content */}
          <Box
            sx={{
              padding: "16px",
              maxHeight: "500px",
              overflowY: "auto",
              backgroundColor: "white",
            }}
          >
            {plantList &&
              plantList.map((plant, index) => (
                <Box
                  key={index}
                  onClick={() => setSelectedPlant(plant)}
                  sx={{
                    padding: "10px",
                    marginBottom: "10px",
                    border: "1px solid #ddd",
                    borderRadius: "4px",
                    cursor: "pointer",
                    backgroundColor:
                      selectedPlant?.SerialID === plant.SerialID
                        ? "#f0f0f0"
                        : "white",
                  }}
                >
                  <Typography variant="h6">{plant.Name}</Typography>
                  <Typography variant="body2">
                    Serial ID: {plant.SerialID}
                  </Typography>

                  <Box
                    sx={{
                      mt: 2,
                      display: "flex",
                      justifyContent: "space-between",
                    }}
                  >
                    <Box>
                      {temperature !== null && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: "8px",
                            mb: 1,
                          }}
                        >
                          <i className="fa-solid fa-temperature-three-quarters"></i>
                          {desiredRange && (
                            <Badge
                              bg={
                                temperature >= desiredRange.min &&
                                temperature <= desiredRange.max
                                  ? "success"
                                  : "danger"
                              }
                            >
                              {temperature} °C
                            </Badge>
                          )}
                        </div>
                      )}

                      {light !== null && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: "8px",
                            mb: 1,
                          }}
                        >
                          <i className="fa-solid fa-sun"></i>
                          {lightRange && (
                            <Badge
                              bg={
                                light >= lightRange.min &&
                                light <= lightRange.max
                                  ? "success"
                                  : "danger"
                              }
                            >
                              {light} lux
                            </Badge>
                          )}
                        </div>
                      )}
                    </Box>

                    <Box>
                      {moisture !== null && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: "8px",
                            mb: 1,
                          }}
                        >
                          <i className="fa-solid fa-droplet"></i>
                          {moistureRange && (
                            <Badge
                              bg={
                                moisture >= moistureRange.min &&
                                moisture <= moistureRange.max
                                  ? "success"
                                  : "danger"
                              }
                            >
                              {moisture} %
                            </Badge>
                          )}
                        </div>
                      )}

                      {soilHumidity !== null && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: "8px",
                            mb: 1,
                          }}
                        >
                          <i className="fa-solid fa-glass-water"></i>
                          {soilHumidityRange && (
                            <Badge
                              bg={
                                soilHumidity >= soilHumidityRange.min &&
                                soilHumidity <= soilHumidityRange.max
                                  ? "success"
                                  : "danger"
                              }
                            >
                              {soilHumidity} %
                            </Badge>
                          )}
                        </div>
                      )}
                    </Box>
                  </Box>
                </Box>
              ))}
          </Box>
        </Box>

        {/* Graph */}
        <Box
          sx={{
            flex: "1 1 50%", // Thêm flex basis và growth
            minWidth: "300px", // Thêm min-width
            border: "1px solid #ddd",
            borderRadius: 3,
            p: 2,
          }}
        >
          <LineChart
            dataset={dataset}
            xAxis={[
              {
                id: "Years",
                dataKey: "date",
                scaleType: "time",
                valueFormatter: (date) => date.getFullYear().toString(),
              },
            ]}
            series={[
              {
                id: "France",
                label: "OK",
                dataKey: "fr",
                stack: "total",
                area: true,
                showMark: false,
              },
              {
                id: "Germany",
                label: "Not OK",
                dataKey: "dl",
                stack: "total",
                area: true,
                showMark: false,
              },
            ]}
            height={500}
          />
        </Box>
      </Box>
    </Box>
  );
}

export default Dashboard;
